...
function ActivitySkeleton() {
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      <div className="animate-pulse rounded-xl border border-border bg-card p-5 lg:col-span-2">
...
export default function RecentActivity({ tasksByStatus }: RecentActivityProps) {
  ...
  // Status badge counts
  const statuses = [
    { label: 'K řešení', count: tasksByStatus.todo.length, color: 'bg-amber-400/15 text-amber-400' },
    { label: 'Probíhá', count: tasksByStatus.in_progress.length, color: 'bg-blue-400/15 text-blue-400' },
    { label: 'Hotovo', count: tasksByStatus.done.length, color: 'bg-emerald-400/15 text-emerald-400' },
  ]
        ...
        {urgentTasks.length === 0 ? (
          ...
          <ul className="space-y-2">
            {urgentTasks.map(task => (
              <li key={task.id} className="rounded-lg border border-border/50 bg-muted/30 p-2.5">
